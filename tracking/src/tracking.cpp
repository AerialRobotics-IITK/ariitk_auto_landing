...
void Tracking::run() {
    ...
    if (!using_detection_) {
        if((fabs(platform_odom_.pose.pose.position.x-quad_odom_.pose.pose.position.x) < 0.1) 
            && (fabs(platform_odom_.pose.pose.position.y-quad_odom_.pose.pose.position.y) < 0.1)) {
            ROS_INFO("Over platform.");
        }
        
        if((fabs(platform_cmd_vel_[0].linear.x) > 0.0001 || fabs(platform_cmd_vel_[0].linear.y) > 0.0001)) {
            updateSetPoint();
        }
...
void Tracking::updateSetPoint() {
    ...
    if( (fabs(distance_x) > max_x_) || (fabs(distance_y) > max_y_) ){
        ...
        if((time_[1] - time_[0]) > min_time_) {
            ...
            if(norm > max_norm_) {
                approximation_value[1] = distance_approximation_value_ - norm;
            }
            double x_approx = platform_cmd_vel_[1].linear.x * inv_state_publish_rate_ + 0.5 * (platform_cmd_vel_[1].linear.x - platform_cmd_vel_[0].linear.x) * pow(inv_state_publish_rate_, 2);
            double y_approx = platform_cmd_vel_[1].linear.y * inv_state_publish_rate_ + 0.5 * (platform_cmd_vel_[1].linear.y - platform_cmd_vel_[0].linear.y) * pow(inv_state_publish_rate_, 2);
            ...
            setpt_.pose.position.x = platform_odom_.pose.pose.position.x + approximation_value[0] * x_approx + approximation_value[1] * setpt_approximation_x;
            setpt_.pose.position.y = platform_odom_.pose.pose.position.y + approximation_value[0] * y_approx + approximation_value[1] * setpt_approximation_y;
        }
        else {
            setpt_.pose.position.x = platform_odom_.pose.pose.position.x;
            setpt_.pose.position.y = platform_odom_.pose.pose.position.y;
