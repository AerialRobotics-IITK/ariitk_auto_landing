...
void Tracking::init(ros::NodeHandle& nh, ros::NodeHandle& nh_private, char** argv) {
	...
	inv_state_publish_rate_ = 1.0 / publish_rate;
...
void Tracking::run() {
	if (!using_detection_) {
		if (isPlatformMoving()) { updateSetPoint(); }
...
void Tracking::modelStateCallback(const gazebo_msgs::ModelStates& msg) {
	...
	platform_odom_.pose.pose.position = msg.pose[index].position;

	platform_cmd_vel_[0].linear = platform_cmd_vel_[1].linear;

	platform_cmd_vel_[1].linear = msg.twist[index].linear;
}
...
void Tracking::platformOdometryCallback(const nav_msgs::Odometry& msg) {
	...
	platform_cmd_vel_[1].linear = msg.twist.twist.linear;
}
