...
cv::Mat platform_detect::preprocess(cv::Mat& img,
    ...
    std::vector<double>& dist_coeff,#include <template/detect_platform.hpp>
...
namespace ariitk::detect
{
    ...
    void platform_detect::imageCb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        {
        if(cv::arcLength(cv::Mat(list_contours[i]), true) >7500 || cv::arcLength(cv::Mat(list_contours[i]), true)<250-(quad_height-4)*20)
        continue;
       ...
       /* for(int i = 0; i < list_contours.size(); i++)
        ...
        if (std::fabs(cv::arcLength(cv::Mat(hull[i]), true)) < 100)
            continue;
        cv::drawContours(drawing, list_contours, i, color_contours, 1, 8, hierarchy, 0, cv::Point());
       ...
       // cv::waitKey(0);       
        ...
        {
        if (std::fabs(cv::arcLength(cv::Mat(hull[i]), true)) < contour_perimeter_thresh-(quad_height)*contour_perimeter_scale) //excluding the quad shadow
            continue;
        cv::approxPolyDP(cv::Mat(hull[i]), approx, cv::arcLength(cv::Mat(hull[i]), true)*error_limit, true); // Approximate contour with accuracy proportional to the contour perimeter
        if(approx.size()==4)
        {
            double d1,d2,d3,d4;
            d1=sqrt(pow(double(approx[0].x-approx[1].x),2)+pow(double(approx[0].y-approx[1].y),2));
            d2=sqrt(pow(double(approx[2].x-approx[3].x),2)+pow(double(approx[2].y-approx[3].y),2));
            d3=sqrt(pow(double(approx[1].x-approx[2].x),2)+pow(double(approx[1].y-approx[2].y),2));
            d4=sqrt(pow(double(approx[3].x-approx[0].x),2)+pow(double(approx[3].y-approx[0].y),2));
    ...
    void platform_detect::run()
    {
        platform_centre_pub.publish(center);
    }
}
    bool is_undistort) {
	ROS_ASSERT(img.empty() != true);
