...
namespace ariitk::detect
{
    ...
    void platform_detect::imageCb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        {
        if (std::fabs(cv::contourArea(hull[i])) < 6000) //excluding the quad shadow
            continue;
        cv::approxPolyDP(cv::Mat(hull[i]), approx, cv::arcLength(cv::Mat(hull[i]), true)*0.1, true); // Approximate contour with accuracy proportional to the contour perimeter
        if(approx.size()==4)
        {
            double d1,d2,d3,d4;
